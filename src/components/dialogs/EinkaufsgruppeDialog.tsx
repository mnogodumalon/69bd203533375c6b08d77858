...
interface EinkaufsgruppeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Einkaufsgruppe['fields']) => Promise<void>;
  defaultValues?: Einkaufsgruppe['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function EinkaufsgruppeDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: EinkaufsgruppeDialogProps) {
  const [fields, setFields] = useState<Partial<Einkaufsgruppe['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'einkaufsgruppe');
      await onSubmit(clean as Einkaufsgruppe['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Einkaufsgruppe['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Einkaufsgruppe bearbeiten' : 'Einkaufsgruppe hinzufügen';
