...
export function ArtikelDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: ArtikelDialogProps) {
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "artikelname": string | null, // Artikelname\n  "kategorie": LookupValue | null, // Kategorie (select one key: "obst_gemuese" | "milchprodukte" | "fleisch_fisch" | "backwaren" | "getraenke" | "tiefkuehl" | "konserven" | "hygiene" | "sonstiges") mapping: obst_gemuese=Obst & Gemüse, milchprodukte=Milchprodukte, fleisch_fisch=Fleisch & Fisch, backwaren=Backwaren, getraenke=Getränke, tiefkuehl=Tiefkühlprodukte, konserven=Konserven & Trockenware, hygiene=Hygiene & Haushalt, sonstiges=Sonstiges\n  "einheit": LookupValue | null, // Einheit (select one key: "stueck" | "kg" | "gramm" | "liter" | "milliliter" | "packung" | "flasche" | "dose" | "beutel") mapping: stueck=Stück, kg=Kilogramm (kg), gramm=Gramm (g), liter=Liter (l), milliliter=Milliliter (ml), packung=Packung, flasche=Flasche, dose=Dose, beutel=Beutel\n  "artikelbeschreibung": string | null, // Beschreibung\n}`;
      ...
      if (file.type.startsWith('image/') || file.type === 'application/pdf') {
        try {
          ...
          setFields(prev => ({ ...prev, artikelbild: fileUrl }));
        } catch (uploadErr) {
