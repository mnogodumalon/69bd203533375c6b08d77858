...
import type { Einkaufsgruppe, Teilnehmer, Einkaufsliste, Artikel, CreateEinkaufsgruppe, CreateTeilnehmer, CreateEinkaufsliste, CreateArtikel } from '@/types/app';
...
export class LivingAppsService {
  // --- EINKAUFSGRUPPE ---
  static async getEinkaufsgruppe(): Promise<Einkaufsgruppe[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.EINKAUFSGRUPPE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Einkaufsgruppe[];
    return enrichLookupFields(records, 'einkaufsgruppe');
  }
  static async getEinkaufsgruppeEntry(id: string): Promise<Einkaufsgruppe | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.EINKAUFSGRUPPE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Einkaufsgruppe;
    return enrichLookupFields([record], 'einkaufsgruppe')[0];
  }
  static async createEinkaufsgruppeEntry(fields: CreateEinkaufsgruppe) {
    return callApi('POST', `/apps/${APP_IDS.EINKAUFSGRUPPE}/records`, { fields: cleanFieldsForApi(fields as any, 'einkaufsgruppe') });
  }
  static async updateEinkaufsgruppeEntry(id: string, fields: Partial<CreateEinkaufsgruppe>) {
    return callApi('PATCH', `/apps/${APP_IDS.EINKAUFSGRUPPE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'einkaufsgruppe') });
  }
  static async deleteEinkaufsgruppeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.EINKAUFSGRUPPE}/records/${id}`);
  }
